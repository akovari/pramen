...
| Phases 2–3 | Phase 2 Group X1 done offline: X1.1–X1.8 (Wasmtime host, `type: wasm`, transform test, Rust guest template, OCI pull-by-digest with allow-list + signature hook; Azure Blob + GCS sources with offline residency-aware validation; `ai.generate` with bounded UTF-8 output; Postgres-backed ledger + checkpoint store). X2.1 done (third-party WASM guest under `examples/external-wasm-guest/` passes `pramen transform test`). Open: X2.2. Cloud-blocked: S2.2 frontier, P2.1 acceptance. Phase 3 not started. |
