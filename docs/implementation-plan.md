...
| Phases 2–3 | Phase 2 done offline: X1.1–X1.8 + X2.1–X2.2 (WASM host/OCI/Azure/GCS/`ai.generate`/Postgres backends; third-party WASM guest under `examples/external-wasm-guest/`; `deploy/` systemd + container profiles, Grafana for real OTLP series, AWS runbook + site cookbook — live apply credentials-gated). Cloud-blocked: S2.2 frontier, P2.1 acceptance. Phase 3 research (E2) starting offline. |
