...
| Area | State |
| --- | --- |
| Milestone T | T1.1–T1.5, T1.7 done (T1.7: Criterion benches + `scripts/perf-gate.sh`, a merge-base-vs-HEAD regression gate failing the Perf workflow when the lower 95% CI bound of the change exceeds +5% on designated benches). T1.6: strategy decided (ADR 0005) and L1/L2 patterns proven across the suite (protocol stubs, testcontainers Postgres, MinIO); shared fixtures not extracted into a common crate. |
...
- **P1.8** Bedrock batch adapter from S2.1: staging lifecycle, manifest
  ...
  *Partial 2026-07-12: the provider-batch operator (buffer, submit,
  poll, join, crash reconciliation) and the OpenAI batch adapter
  (Files + Batches APIs in `openai-compat`) are done and stub-tested
  offline; the Bedrock batch leg (S3 staging + model-invocation jobs)
  remains.*
