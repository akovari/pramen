...
| 2 | **E1.3** Fan-out DAGs | Spec + runtime; no cloud |
...
Just completed offline research track: **E2.4** reproducibility harness
(`mise run reproduce` / `reproduce-check`). **E2.5** waits on a venue ADR.
...
| Phases 2–3 | Phase 2 done offline: X1.1–X1.8 + X2.1–X2.2. Phase 3 research offline: E2.1 (dispatch cost model + mock frontier; live frontier → S2.2), E2.2 (memoization contract + measured reuse), E2.3 scaffolding (orientation + scoreboard + `compare/` harnesses; measured competitor legs open), **E2.4 done** (one-command `mise run reproduce` / CI `reproduce-check` + AE checklist). Open: E1.*, E2.3 measured legs, E2.5 (venue ADR + paper). Cloud-blocked: S2.2 frontier, P2.1 acceptance. |
