...
| Phases 2–3 | Phase 2 started: X1.1–X1.3 done (Wasmtime host, `type: wasm` in spec + `pramen run`, `pramen transform test`, Rust guest template in `templates/wasm-transform-rust/`). X1.7 done (`ai.generate` with required UTF-8 `maxChars` + `maxOutputTokensPerRecord`, provider cap + post-validation reject, work-key differentiated from extract/classify). Open without AWS: X1.4 OCI, X1.5 Azure/GCS, X1.8 shared backends. Cloud-blocked: S2.2 frontier, P2.1 acceptance. Phase 3 not started. |
