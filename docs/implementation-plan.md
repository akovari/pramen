...
| Group P1 | Two runnable verticals merged. Deterministic: `pramen run` executes Parquet/NDJSON → SQL → Postgres end to end (verified with 200k rows). Governed AI: `ai.extract`/`ai.classify` run against the production SQLite ledger with pre-dispatch budgets, strict output validation, and `onInvalid` policies — verified end to end (NDJSON → SQL → ai.classify → Postgres) with full ledger reuse on replay. In: slices of P1.1/P1.2 (local Parquet + NDJSON sources), P1.4 (append COPY sink), P1.5 (provider trait + capabilities), P1.6 (ledger + pinned canonicalization), P1.9 (OpenAI-compatible adapter, L1 stub-tested), P1.10 (schema generation + validation), P1.11 (pre-dispatch input budgets, provider-side output caps), P1.12 (sequential operator), P1.13 (per-batch SQL), P1.15 (validate/explain/run + `ai status`). Open: P1.3 checkpoints, P1.7/P1.8 Bedrock, remote object stores, P1.14, P1.16–P1.20, per-run cost ceilings and spike breakers, review-queue routing. |
