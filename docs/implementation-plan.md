...
| Phases 2–3 | Phase 2 started: X1.1–X1.5 and X1.7 done (Wasmtime host, `type: wasm`, `pramen transform test`, Rust guest template, OCI pull-by-digest with allow-list + signature hook; Azure Blob + GCS sources with offline residency-aware validation; `ai.generate` with required UTF-8 `maxChars` + `maxOutputTokensPerRecord`). Open without AWS: X1.8 shared backends. Cloud-blocked: S2.2 frontier, P2.1 acceptance. Phase 3 not started. |
