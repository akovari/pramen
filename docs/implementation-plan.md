...
| Phases 2–3 | Phase 2 started: X1.1–X1.4 done (Wasmtime host, `type: wasm`, `pramen transform test`, Rust guest template, OCI pull-by-digest with allow-list + signature hook). X1.7 done (`ai.generate` with required UTF-8 `maxChars` + `maxOutputTokensPerRecord`, provider cap + post-validation reject, work-key differentiated from extract/classify). Open without AWS: X1.5 Azure/GCS, X1.8 shared backends. Cloud-blocked: S2.2 frontier, P2.1 acceptance. Phase 3 not started. |
